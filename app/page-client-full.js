...
// Lazy load ALL heavy components with intersection observer
...
const SwiperComponent = dynamic(() => import('swiper/react').then(mod => ({ default: mod.Swiper })), {
  ssr: false
});

const SwiperSlideComponent = dynamic(() => import('swiper/react').then(mod => ({ default: mod.SwiperSlide })), {
  ssr: false
});

export default function HomePageClient({ initialData }) {
  // Extract data from server props
  const postsData = initialData?.postsData || {};
  const banner = initialData?.banner || [];
  const urlData = initialData?.urlData || {};

  // Destructure all post data
  const featuredPosts = postsData?.featured || [];
  const topPost = postsData?.top_post_news || [];
  const popularPosts = postsData?.popular || [];
  const worldNews = postsData?.world_popular_news || [];
  const mixnews = postsData?.mixed_popular_news || [];
  const sports = postsData?.sports_popular_news || [];
  const entertainment = postsData?.entertainment_popular_news || [];
  const literature = postsData?.literature_popular_news || [];
  const opinion = postsData?.opinion_popular_news || [];
  const photoFeature = postsData?.photo_feature_popular_news || [];
  const lifestyle = postsData?.lifestyle_popular_news || [];
  const multimedia = postsData?.multimedia_popular_news || [];
  const bangladeshNews = postsData?.bangladeshi_popular_news || [];
  const politics = postsData?.politics_popular_news || [];
  const crime = postsData?.crime_popular_news || [];
  const economy = postsData?.economy_popular_news || [];
  const technology = postsData?.technology_popular_news || [];
  const country = postsData?.country_popular_news || [];
  const science = postsData?.science_popular_news || [];
  const engineering = postsData?.engineering_popular_news || [];
  const health = postsData?.health_popular_news || [];
  const successStory = postsData?.success_storys_popular_news || [];
  const education = postsData?.education_popular_news || [];
  const environment = postsData?.environment_popular_news || [];
  const interview = postsData?.interview_popular_news || [];
  const corporate = postsData?.corporate_popular_news || [];
